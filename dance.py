...
def display_disco():
    ...
    while True:
        if state["game_over"]:
            ...
            for x in range(8):
                for y in range(8):
                    if random.random() > 0.5:  # 50% chance to flash
...
def get_audio_pitch():
    """Get the pitch of the audio signal."""
    data = np.frombuffer(stream.read(CHUNK, exception_on_overflow=False), dtype=np.int16)
    fft_data = np.abs(fft(data))[:CHUNK // 2]
    freq = np.fft.fftfreq(CHUNK, 1.0 / RATE)[:CHUNK // 2]
    pitch = freq[np.argmax(fft_data)]
    return pitch

def display_disco_with_pitch():
    """Display disco-like effects with flashing colors based on audio pitch."""
    global state
    while True:
        pitch = get_audio_pitch()
        for x in range(8):
            for y in range(8):
                if random.random() > 0.5:  # 50% chance to flash
                    r = int((pitch % 255) * random.random())
                    g = int((pitch % 255) * random.random())
                    b = int((pitch % 255) * random.random())
                    sense.set_pixel(x, y, (r, g, b))
                else:
                    sense.set_pixel(x, y, (0, 0, 0))
        time.sleep(0.1)
...
# Start displaying disco mode with pitch detection in a separate thread
disco_thread = threading.Thread(target=display_disco_with_pitch)
